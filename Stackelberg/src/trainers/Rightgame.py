...
class RightgameTrainer(BaseTrainer):
    ...
    def __init__(self, options, dataset):
        ...
        super(RightgameTrainer, self).__init__(options, dataset, worker=worker)
    ...
    def compute_prob(self, duipai=False):
        # here is real
        print('comput prob right')
        sum = 0
        for c in self.clients:
            sum += (c.pk*c.pk/c.ck)
        val_p = math.sqrt(2*self.logDict['budget']/sum)
        for c in self.clients:
            self.logDict['Pk'].append(val_p * c.pk)
            c.qk = 0.5*val_p*c.pk/c.ck
            if c.qk > 1:
                c.qk = 1
            self.logDict['qk'].append(c.qk)

        # print(self.logDict['qk'])
        print(np.array(self.logDict['qk']).mean())
