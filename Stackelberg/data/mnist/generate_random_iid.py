...
'''
        - Generate random iid-x
        - run program in mnist/
        - 
'''
...
def main():
    ...
    for number in range(10):
        ...
        #   shuffle 
        data = train_mnist.data[idx]
        np.random.shuffle(data)
        mnist_traindata.append(data)
        # mnist_traindata.append(train_mnist.data[idx])

    ## train data info
    for number in range(10):
        print("TRAIN DATA LABEL%d has %dDATA" % (number, len(mnist_traindata[number]) ) )
    ...
    for number in range(10):
        ...
        #   shuffle
        data = test_mnist.data[idx]
        np.random.shuffle(data)
        mnist_testdata.append(data)
    ...
    num_samples = np.random.lognormal(5, 1, (NUM_USER)) + 50
    ...
    train_path = './data/train/iid%d_IMG%d.json' % (IID, image)
    test_path = './data/test/iid%d_IMG%d.json' % (IID, image)
